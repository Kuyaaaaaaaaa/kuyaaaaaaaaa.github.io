...
class Notation {
    ...
    isInvalid(decimalValue) {
        if (decimalValue === "NaN") return false;
    ...
    getSB3Layer(decimalValue) {
        ...
        if (decimalValue.lt(1)) {
            return decimalValue.sub(1);
        }

        if (decimalValue.lt(10)) {
            return decimalValue.log10();
        }
        ...
        return layer.add(innermost.log10().log10()).add(1);
...
// the one and only. also i might move these dang lists to the top
...
class StandardNotation extends Notation {
    formatValue(decimalValue) {
        let firstTen = ["K","M","B","T","Qa","Qi","Sx","Sp","Oc","No","De"];
        let firstLayer = ["","u","d","t","qa","qi","sx","sp","oc","no","","De","Vg","Tg","Qd","Qg","Sg","St","Og","Nn","","Ce","Dn","Tc","Qe","Qu","Sc","Si","Oe","Ne"]
        ...
        if (decimalValue.lt(E("ee36"))) {
            let tier = decimalValue.log10().div(3).sub(1).log10().div(3).sub(1).floor();
            let notatedNumberSquared = tier.toNumber();

            let notatedNumber = decimalValue.log10().div(tier.pow_base(1000)).div(3).floor().toNumber();

            let layer1Part = decimalValue.div(decimalValue.log10().div(3).floor().mul(3).pow_base(10)).toFixed(2);
...
class PickleDoughNotation extends Notation {
    formatValue(decimalValue) {
        ...
        const imInABigPickle = smallNumber.mul(moderateNumber.pow_base(10)).log(100).mul(2000).pow(0.9).floor().toNumber();
...
class LogarithmNotation extends Notation {
    ...
    formatValue(decimalValue) {
        ...
        return "e" + decimalValue.log10().toFixed(2);
    }
}

// I had to go through with googleai's aah saying "What sexuality do you RESONATE with the most" well of course its antimatter

class InventionNotation extends Notation {
    formatValue(decimalValue) {
        const absolutelyCollosalPrefixesFromATOZ = ["a", "abro", "aego", "allo", "akoi", "andro", "auto", "bi", "cetero", "cupio", "demi", "fray", "fin", "gyno", "hetero", "homo", "min", "mono", "multi", "nin", "omni", "pan", "pluri", "poly", "recipro", "skolio", "sapio"];
        let representation = "";
        if (decimalValue.eq(0)) {
            return "straight";
        }
        ...
        let keyValue = this.getSB3Layer(decimalValue).mul(250).pow(1.7).floor().toNumber();

        if (decimalValue.lt(1)) {

                keyValue = this.getSB3Layer(decimalValue.recip()).mul(250).pow(1.7).floor().toNumber();

                representation = "anti";
        }
        
        if (Math.floor(keyValue) == 0) {
            return "gay";  // apparently means 1 :sus:
        }

        return representation + findByIndex(Math.floor(keyValue), absolutelyCollosalPrefixesFromATOZ) + "sexual";
    }
